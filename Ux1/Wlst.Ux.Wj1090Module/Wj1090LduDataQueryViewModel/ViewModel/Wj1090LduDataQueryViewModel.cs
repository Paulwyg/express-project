...
namespace Wlst.Ux.Wj1090Module.Wj1090LduDataQueryViewModel.ViewModel
{
    ...
    public partial class Wj1090LduDataQueryViewModel
    {
        public void InitAction()
        {
            ProtocolServer.RegistProtocol(
                Sr.ProtocolPhone .LxLdu .wst_ldu_data ,// .wlst_svr_ans_cnt_wj1090_request_data ,//.ClientPart.wlst_Wj1090_server_ans_clinet_request_Data,
                ResolveRequestData,
                typeof (Wj1090LduDataQueryViewModel), this);
        }

        private void ResolveRequestData(string session,Wlst .mobile .MsgWithMobile  infos)
        {
            var info = infos.WstLduDatas;
            if (info == null) return;
            Items.Clear();
            int i = 0;
            ...
            Remind = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") +"  线路数据查询成功，共计" + Items.Count + " 条数据.";
        }
    ...
    public partial class Wj1090LduDataQueryViewModel
    {
        private void RequestData()
        {
           ...
           // Remind = "查询命令已发送...";
            var info = Sr.ProtocolPhone.LxLdu.wst_ldu_data;// .wlst_cnt_wj1090_request_data ;//.ServerPart.wlst_Wj1090_clinet_request_Data;
            if (IsLockTml)
            {
               // info.WstLduHisData  .AttachRtuId = AttachId;
                info.WstLduDatas.RtuId = RtuId;
                info.WstLduDatas.LineIds.Add(LduLineId);// = LduLineId;
            }
            else
            {
                //info.WstLduHisData.AttachRtuId = AttachId;
                info.WstLduDatas.RtuId = 0;
                //info.WstLduHisData.LoopId = 0;
            }
            info.WstLduDatas .DtStart = tStartTime.Ticks ;
            info.WstLduDatas.DtEnd = tEndTime.Ticks;
