...
using Wlst.Ux.Wj1080Module.Resources;
        ...
        public override void AddChild()
        {
            ...
            if (NodeType == TypeOfTabTreeNode.IsGrp)
            {
                ...
                foreach (var f in gprs)
                {
                    ...
                    if (para.EquipmentType == WjParaBase.EquType.Lux && para.RtuFid == 0)//是电表，而且是主设备
                    {
            ...
            if (NodeType == TypeOfTabTreeNode.IsGrpSpecial)
            {
                ...
                foreach (var f in gprs)
                {
                    var para = Wlst.Sr.EquipmentInfoHolding.Services.EquipmentDataInfoHold.GetInfoById(f);
                    if (para == null) continue;//|| para.EquipmentType != WjParaBase.EquType.Mru
                    if (para.EquipmentType == WjParaBase.EquType.Lux && para.RtuFid == 0) //电表为主设备
                    {
                        this.ChildTreeItems.Add(new TreeNodeWj1080ViewModel(para.RtuId, para.RtuName, para.RtuFid));
                    }
