      ...
      //  public static bool UserManalOperator = false;
        ...
        public static void ShowThisView(int dataview)
        {
            ...
            Wlst.Cr.Core.CoreServices.RegionManage.ShowViewByIdAttachRegionWithArgu(
                Ux.Wj2096Module.Services.ViewIdAssign.NewDataGridViewId, dataview);
        }
    ...
    public partial class PartolViewVm
    {
        private void InitAciton()
        {
            ProtocolServer.RegistProtocol(
                Wlst.Sr.ProtocolPhone .LxSluSgl.wst_slu_sgl_ctrl_measure, OnSluMeasure,
                typeof (PartolViewVm), this,true);
        }
        ...
        private void OnSluMeasure(string sessionid,Wlst .mobile .MsgWithMobile info)
        {
            ...
            var DataItemTmp = new ObservableCollection<DataSluLamp5>();
            ...
            if (data.InfoBaseic5 != null && data.InfoBaseic5.Count > 0)
            {
                ...
                foreach (var g in data.InfoBaseic5)
                {
                    ...
                    foreach (var f in g.Items)
                    {
                        ...
                        List<DataSluLamp5> ctrllist = new List<DataSluLamp5>();
                        foreach (var i in DataItem5)
                        {
                            ctrllist.Add(i);
                        }
                        ...
                        {
                            if (t.CtrlId == f.CtrlId && t.LampId == f.LampId)
                            {
                                this.DataItem5.Remove(t);
                                //flg = false;
                            }
                        }

                        DataItemTmp.Add(new DataSluLamp5(f, sluId, g.Info.CtrlId, g.Info.Status) { DateCtrlCreate = dts });
                ...
                //排序
                var ttt = (from t in DataItemTmp orderby t.SluId, t.CtrlId, t.LampId select t).ToList();
                foreach (var g in ttt)
                {
                    this.DataItem5.Add(g);
                }


                Remind += "共"+DataItem5.Count +"条数据";
