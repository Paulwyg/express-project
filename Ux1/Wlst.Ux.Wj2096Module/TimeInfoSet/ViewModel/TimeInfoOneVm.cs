...
namespace Wlst.Ux.Wj2096Module.TimeInfoSet.ViewModel
{
    public partial class TimeInfoOneVm : Wlst.Cr.Core.CoreServices.ObservableObject
    {
        public TimeInfoOneVm()
        {
            ...
            Ctrls = new Dictionary<int, VSluTimeScheme.VSluTimeSchemeItem.VSluTimeCtrlSluOne>();
        }

        public TimeInfoOneVm(int areaId, VSluTimeScheme.VSluTimeSchemeItem info)
        {
            CmdPwmValueStrs = "0%";
            UsedSluCount = info.SluCtrls.Count;
            //this.SchemeDesc = info.SchemeDesc;
            this.SchemeId = info.SchemeId;
            this.SchemeName = info.SluTimePlanInfo.SchemeName;
            this.CmdType = info.SluTimePlanInfo.CmdType;
            //this.IsSluOrCtrlScheme = info.IsSluOrCtrlScheme;
            this.IsNotUsed = info.SluTimePlanInfo.IsNotUsed;
            ...
            this.OperationMethod = info.SluTimePlanInfo.OperationMethod == 3 ? 11 : info.SluTimePlanInfo.OperationMethod;
            ...
            //}
            if (info.SluTimePlanInfo.OperationMethod == 3)
            {
            ...
            {
                var para = SluSglFieldGrpHold.MySlef.Get(t.VSluId);
                //if (Ctrls.ContainsKey(t.VSluId)) Ctrls[t.VSluId] = t;

                if (t.OperatorType == 3)
                {
                    var sluCtrl = new VSluTimeScheme.VSluTimeSchemeItem.VSluTimeCtrlSluOne() { VSluId = t.VSluId, OperatorType = 4, CtrlOrGrp = new List<int>() };
                    var xx = t.CtrlOrGrp.ToList();
                    if (Ctrls.ContainsKey(t.VSluId))
                    xx.AddRange(Ctrls[t.VSluId].CtrlOrGrp);
                    sluCtrl.CtrlOrGrp = xx;
                    Ctrls[t.VSluId] = sluCtrl;
                }
                else if (t.OperatorType == 2)
                    foreach (var f in para)
                    {
                        var sluCtrl = new VSluTimeScheme.VSluTimeSchemeItem.VSluTimeCtrlSluOne() { VSluId = t.VSluId, OperatorType = 4, CtrlOrGrp = new List<int>()};
                        if (t.CtrlOrGrp.Contains(f.GrpId))
                        {
                            sluCtrl.CtrlOrGrp.AddRange(f.CtrlLst);
                            if(Ctrls.ContainsKey(t.VSluId))
                            sluCtrl.CtrlOrGrp.AddRange(Ctrls[t.VSluId].CtrlOrGrp);
                            Ctrls[t.VSluId] = sluCtrl;
                        }
                    }
                else if(t.OperatorType == 1)
                    Ctrls[t.VSluId] = t;
        ...
        public Dictionary<int, VSluTimeScheme.VSluTimeSchemeItem.VSluTimeCtrlSluOne> Ctrls;

        public VSluTimeScheme.VSluTimeSchemeItem BackToSluTimeSchemeOne()
        {
            var infof = new VSluTimeScheme.VSluTimeSchemeItem()
                            {
                                SluTimePlanInfo = new VSluTimeScheme.VSluTimeSchemeItem.VSluTimePlan()   
                                                    {
                                ...
                                SluCtrls = new List<VSluTimeScheme.VSluTimeSchemeItem.VSluTimeCtrlSluOne>(),
                            };
            infof.SluTimePlanInfo.SchemeName = SchemeName;
            infof.SluTimePlanInfo.IsNotUsed = IsNotUsed;
            ...
            {
                infof.SluTimePlanInfo.OperationMethod = 3;
            ...
            {
                if (g.Value.OperatorType == 1)
                {
                    infof.SluCtrls.Add(g.Value);
                    continue;
                }
                var sluCtrl1 = new VSluTimeScheme.VSluTimeSchemeItem.VSluTimeCtrlSluOne { VSluId = g.Key, OperatorType = 2 ,CtrlOrGrp = new List<int>()};
                var sluCtrl2 = new VSluTimeScheme.VSluTimeSchemeItem.VSluTimeCtrlSluOne { VSluId = g.Key, OperatorType = 3, CtrlOrGrp = new List<int>()};
                var para = SluSglFieldGrpHold.MySlef.Get(g.Key);
                foreach (var t in para)
                {
                    var i = 0;
                    foreach (var f in t.CtrlLst)
                    {
                        if (g.Value.CtrlOrGrp.Contains(f))
                            i++;
                    }
                    if (i == t.CtrlLst.Count)
                    {
                        sluCtrl1.CtrlOrGrp.Add(t.GrpId);
                        foreach (var f in t.CtrlLst)
                        {
                            g.Value.CtrlOrGrp.Remove(f);
                        }
                        
                    }
                }
                sluCtrl2.CtrlOrGrp.AddRange(g.Value.CtrlOrGrp);
                if (sluCtrl1.CtrlOrGrp.Count != 0)
                    infof.SluCtrls.Add(sluCtrl1);
                if (sluCtrl2.CtrlOrGrp.Count != 0)
                    infof.SluCtrls.Add(sluCtrl2);
                //infof.SluCtrls.Add(g.Value);
            }
