...
namespace Wlst.Ux.WJ3005Module.ZOrders.OpenCloseLight.ForSingleTreeGrpMenu
{
    ...
    public class OpenLightLoopControllerForSingleTreeGrpRightMenuBase : MenuItemBase
    {
        /// <summary>
        /// 最底层开灯服务，无继承不可用 需实现LoopId赋值
        /// </summary>
        public OpenLightLoopControllerForSingleTreeGrpRightMenuBase()
        {
            Id = -1;
            Description = "最底层开灯服务，无继承不可用，类型为单终端分组右键菜单。";
            Tag = "开灯最底层服务";
            this.Text = "Null";
        ...
        private void Ex()
        {
            ...
            if (grpInfo != null)
            {
                ...
                if (gprId < 1) return;
                var lst = Wlst.Sr.EquipmentInfoHolding.Services.ServicesGrpSingleInfoHold.GetGrpTmlList(areaId, gprId);
                ...
                //SndOrderServer.OrderSnd(WJ3005Module.Services.EventIdAssign.OpenLight, lst, data, gid);
            ...
            else //地区分组 lvf 2019年5月15日14:26:45
            {
                ...
                LogInfo.Log("地区分组" + " 分组开K" + LoopId + ",开灯命令已经发送");
                Wlst.Cr.CoreMims.ShowMsgInfo.ShowNewMsg.AddNewShowMsg(
                   0, "地区分组", OperatrType.UserOperator, "分组开K" + LoopId);
