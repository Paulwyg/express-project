...
namespace Wlst.Ux.Wj2090Module.Wj2090InfoSet.ViewModel
{
    ...
    public class TreeItemGrplViewModel : ObservableObject
    {
        ...
        private Wlst .client .SluRegulatorGroupParameter   GrpInfo = null;
       ...
       // private int SluId;
        public TreeItemGrplViewModel(object targetInfomation,int sluId, bool IsGrp)
        {
            ...
            if (!IsGrp)
            {
                var terminalInfomation = targetInfomation as Wlst .client .SluRegulatorParameter   ;
                ...
                {
                    this.NodeName = terminalInfomation.LampCode ;
                    //ImagesSource = ImageResources.GroupIcon;
                    this.NodeType = TreeNodeType.SluNode ;
                    this.NodeId = terminalInfomation.CtrlPhyId;
                }
            }
            else
            {
                var groupInfomatioin = targetInfomation as Wlst.client.SluRegulatorGroupParameter;
            ...
            BackUpName = "";
            //this.AddChild();
            //  _ChildTreeItems = new ObservableCollection<TreeItemGrplViewModel>();
        ...
        public void AddChild()
        {
            ...
            var infos = Sr.EquipmentInfoHolding.Services.EquipmentDataInfoHold .GetInfoById( SluId);
            if (infos == null) return;
            var tmpsss = infos as  Wlst.Sr .EquipmentInfoHolding .Model .Wj2090Slu ;
            if (tmpsss == null) return;
            foreach (var g in GrpInfo.CtrlPhyLst)
            {
                foreach (var f in tmpsss.WjSluCtrls  .Values )
                {
                    if (f.CtrlPhyId != g) continue;
                    this.ChildTreeItems.Add(new TreeItemGrplViewModel(f,SluId ,
                                                                      false));
                }
            }
        }
