      ...
      public string PwFactor
      {
        ...
        public DataSluCtrlLampVm(Wlst.client.SluCtrlDataMeasureReply.DataSluCtrlLamp info, int states)
        {
            ...
            else
            {
                ...
                this.ActivePower = (info.ActivePower*bs).ToString("f2");
                ...
                {
                    var x = (info.ActivePower*bs)/(info.Voltage*info.Current);
                    if (x > 1 && x < 1.2) x = 1;
            ...
            {
                var t =
                    Sr.EquipmentInfoHolding.Services.EquipmentDataInfoHold.InfoItems[info.SluId]
                    as Wlst.Sr.EquipmentInfoHolding.Model.Wj2090Slu;

                if (t != null && t.WjSluCtrls.ContainsKey(info.CtrlId))
                {
                    DBCode = t.WjSluCtrls[info.CtrlId].BarCodeId.ToString("");
                    DBLamp = t.WjSluCtrls[info.CtrlId].LampCode;
                }
