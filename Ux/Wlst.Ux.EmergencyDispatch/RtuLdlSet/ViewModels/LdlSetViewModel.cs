...
using Wlst.Ux.EmergencyDispatch.RtuLdlSet.Services;
using Wlst.client;
        ...
        void NextStep(List<int> rtus)
        {
                ...
                )
            {
                foreach (var g in t.Value.LstTml)
                {
                    ...
                    tmpdir.Add(g, exadd ? t.Value.AreaId + "-" + t.Key + "-" + t.Value.GroupName : t.Key + "-" + t.Value.GroupName);
                }
            }
