...
using Wlst.Ux.EmergencyDispatch.ControlCenterManagDemo2.ViewModel;
using Wlst.client;
...
namespace Wlst.Ux.EmergencyDispatch.ControlCenterManagDemo2.Views
{
    ...
    public partial class ShutDownOrReRunWindow : CustomChromeWindow
    {
        ...
        private void CmdSave(object sender, RoutedEventArgs e)
        {
            if(_isShutDownOrReRun)
            {
                var info = Sr.ProtocolPhone .LxRtu .wst_rtu_orders ;// .wlst_cnt_wj3090_order_snd_paras ;//.ServerPart.wlst_rtuargsupdate_clinet_order_stopruning;
            ...
            {
                var info = Sr.ProtocolPhone.LxRtu .wst_rtu_orders ;//.ServerPart.wlst_rtuargsupdate_clinet_order_stopruning;
                //info.Args.Addr.AddRange((from t in Items select t.NodeId).ToList());
