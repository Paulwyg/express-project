...
namespace Wlst.Ux.CrissCrossEquipemntTree.GrpSingleTabShowViewModel.ViewModels
{
    ...
    /// </summary>
    public class TreeNodeItemRegionGroupViewModel : TreeNodeBaseNode 
    {
        ...
        //public TreeNodeItemRegionGroupViewModel()
        //{
        //    this.NodeType = TypeOfTabTreeNode.IsGrp;
        //    //Visi = Visibility.Visible;
        //}

        public TreeNodeItemRegionGroupViewModel(TreeNodeBaseNode mvvmFather, int areaId, int groupId, TypeOfTabTreeNode type)
        {
        ...
        public TreeNodeItemRegionGroupViewModel(TreeNodeBaseNode mvvmFather,int areaId,int grpId,string grpName, TypeOfTabTreeNode type)
        {
            ...
            if (type == TypeOfTabTreeNode.IsGrpSpecial)
            {
                tukey = new Tuple<int, int, int>(areaId,-99, mvvmFather.NodeId);
                nodename = "特殊终端";
                this.ImagesIcon = ImageResources.GroupIcon; // ImageSource.GrpBitmapImage;
                this.NodeId = 0;
            ...
            if (type == TypeOfTabTreeNode.IsGrp )
            {
                tukey = new Tuple<int, int, int>(areaId,grpId, mvvmFather.NodeId);
                this.ImagesIcon = ImageResources.GroupIcon; // ImageSource.GrpBitmapImage;
                this.NodeId = grpId;
            }
            ...
            {
                tukey = new Tuple<int, int, int>(areaId,-99,NodeId);
                this.ImagesIcon = ImageResources.GroupIcon; // ImageSource.GrpBitmapImage;
                this.NodeId = grpId;
            }
            ...
            if (type == TypeOfTabTreeNode.IsRegionSpecial)
            {
                tukey = new Tuple<int, int, int>(areaId,0 , NodeId);
                nodename = "特殊终端";
                this.ImagesIcon = ImageResources.GroupIcon; // ImageSource.GrpBitmapImage;
                this.NodeId = 0;
        ...
        public override void AddChild()
        {
            ...
            {
                var regionGrps = ServiceGrpRegionInfoHold.GetGrpByType(AreaId, 2);
                //先加载全局分组  再加载地区分组
                if (regionGrps == null || regionGrps.Count == 0) return;
                foreach (var f in regionGrps)
                {
                    this.ChildTreeItems.Add(new TreeNodeItemRegionGroupViewModel(this, AreaId, f.Key, f.Value,
                ...
                //该地区下有多少终端没有grpid
                var spLst = Wlst.Sr.EquipmentInfoHolding.Services.ServiceGrpRegionInfoHold.GetSpRtulstByGrpRegionId(AreaId, NodeId, false);
                if (spLst == null) return;
                if (spLst.Count == 0) return;


                this.ChildTreeItems.Add(new TreeNodeItemRegionGroupViewModel(this, AreaId, NodeId, "特殊终端",
            ...
            if (NodeType == TypeOfTabTreeNode.IsGrp)
            {
                ...
                var rtulst = Wlst.Sr.EquipmentInfoHolding.Services.ServiceGrpRegionInfoHold.GetRtulstByGrpRegionId(AreaId, NodeId, _father.NodeId);
                ...
                foreach (var f in sortLst)
                {
                    ...
                    var imageInfo = Wlst.Sr.EquipmentInfoHolding.Services.ServiceGrpRegionInfoHold.GetImageIdByRtuid(f);
            ...
            {
                if (_father == null) return;
                var rtulst = Wlst.Sr.EquipmentInfoHolding.Services.ServiceGrpRegionInfoHold.GetSpRtulstByGrpRegionId(AreaId, _father.NodeId, false);
                if (rtulst == null || rtulst.Count == 0) return;
            ...
            if (NodeType == TypeOfTabTreeNode.IsRegionSpecial)
            {
                ...
                var rtulst = Wlst.Sr.EquipmentInfoHolding.Services.ServiceGrpRegionInfoHold.GetSpRtulstByGrpRegionId(AreaId, 0, true);
                if (rtulst == null || rtulst.Count == 0) return;
            ...
            if (NodeType == TypeOfTabTreeNode.IsArea)
            {
                ...
                this.ChildTreeItems.Add(new TreeNodeItemRegionGroupViewModel(this, AreaId, 0, "全部终端",
                ...
                {
                    this.ChildTreeItems.Add(new TreeNodeItemRegionGroupViewModel(null, AreaId, f.Key, f.Value,
                ...
                //    this.ChildTreeItems.Add(new TreeNodeItemRegionGroupViewModel(this, f.AreaId, f.GroupId,f.GroupName,
                //                                                                 TypeOfTabTreeNode.IsGrp));
                //}
                ...
                var sp = Sr.EquipmentInfoHolding.Services.ServiceGrpRegionInfoHold.GetSpRtulstByGrpRegionId(AreaId, 0, true);

                if (sp.Count > 0)

                    this.ChildTreeItems.Add(new TreeNodeItemRegionGroupViewModel(this, AreaId, 0, "特殊终端",
         ...
         public void UpdateArea()
        {
            ...
            foreach (var t in info.LstTml)
            {
                ...
                ChildTreeItems.Add(new TreeNodeItemRegionGroupViewModel(this, AreaId ,t,TypeOfTabTreeNode.IsGrp ));
            }
            ...
            foreach (var f in this.ChildTreeItems)
            {
             ...
             {
                 this.ChildTreeItems.Add(new TreeNodeItemRegionGroupViewModel(this, AreaId, 0,
                                                                              TypeOfTabTreeNode.IsGrpSpecial));
             }
        ...
        public override void OnNodeSelectActive()
        {
            ...
            if (NodeType == TypeOfTabTreeNode.IsRegion)
            {
                ...
                var info = Wlst.Sr.EquipmentInfoHolding.Services.ServiceGrpRegionInfoHold.GetRtulstByGrpRegionId(AreaId, -1, NodeId);
                ...
                if (info.Count > 0)
                {
                    ...
                    rtus = (from t in info where t.Item1 > 1000000 && t.Item2 < 1100000 select t.Item1).ToList();
            ...
            if (NodeType == TypeOfTabTreeNode.IsAll || NodeType == TypeOfTabTreeNode.IsGrp)
            {
                if (UxTreeSetting.IsSelectGrpMapOnlyShow == false) return;
                var ins = new PublishEventArgs()
                {
        ...
        public void ResetCm()
        {
            ...
            {
                //获取 该地区下所有终端
                var info = Sr.EquipmentInfoHolding.Services.ServiceGrpRegionInfoHold.GetRtulstByGrpRegionId(AreaId,-1, NodeId);
                //var info = Wlst.Sr.EquipmentInfoHolding.Services.ServicesGrpSingleInfoHold.GetGroupInfomation(AreaId, NodeId);
                if (info == null) return;
            ...
            {
                if (_father == null) return;
                var info = Sr.EquipmentInfoHolding.Services.ServiceGrpRegionInfoHold.GetRtulstByGrpRegionId(AreaId, NodeId, _father.NodeId);
