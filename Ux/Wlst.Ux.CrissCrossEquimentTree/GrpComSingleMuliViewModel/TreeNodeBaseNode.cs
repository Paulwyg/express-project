...
namespace Wlst.Ux.CrissCrossEquipemntTree.GrpComSingleMuliViewModel
{
    public class TreeNodeBaseNode : TreeNodeBaseViewModel
    {
        ...
        /// areadid ,第二层,第三层组
        /// </summary>
        public Tuple<int, int, int> tuGrpKey = new Tuple<int, int, int>(0,-1,-1);
        ...
        public string ExtendRtuCount
        {
            get
            {
                ...
                else if (this.NodeType == TypeOfTabTreeNode.IsGrp ||this.NodeType == TypeOfTabTreeNode.IsRegion 
                    ||this.NodeType ==TypeOfTabTreeNode.IsGrpSpecial || this.NodeType ==TypeOfTabTreeNode.IsRegionSpecial)
                {
