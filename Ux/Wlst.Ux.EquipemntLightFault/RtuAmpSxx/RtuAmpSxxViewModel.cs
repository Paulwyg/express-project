...
namespace Wlst.Ux.EquipemntLightFault.EquipmentFaultDefineSettingViewModel.RtuAmpSxx
{
    ...
    public partial class RtuAmpSxxViewModel : Wlst.Cr.Core.CoreServices.ObservableObject, IIRtuAmpSxx
    {
        ...
        {
            return DateTime.Now.Ticks - _dtUpdate.Ticks > 100000000;
        }
                ...
                if (_areacomboboxselected != value)
                {
                    ...
                    foreach (var f in ntsg)
                    {
                        ...
                        if (index % 20 == 0)
                        {
                            ...
                            Remark = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + " 正在加载终端参数，请稍后...";
                        }
                    }
    ...
    public partial class RtuAmpSxxViewModel
    {
        ...
        private void RequestOrUpdateFaultTypeInfo(string session, Wlst.mobile.MsgWithMobile infos)
        {
            ...
            else if (data.Op == 4)
            {
                ...
                Remark = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + " 终端当前设置信息请求成功.";
            }
                ...
                foreach (var f in t.ChildTreeItems)
                {
                    if (f.IsChecked)
                    {
                        ...
                        if (flg)
                        {
                            ...
                            {
                                var ttt = WlstMessageBox.Show("上下限保存失败", "终端 " + f.Rtuid + " 回路"+ f.ValueInt1+" 上下限设置有误！是否继续检查？", WlstMessageBoxType.YesNoCancel);
                                if (ttt == WlstMessageBoxResults.Cancel) return;
                                else if (ttt== WlstMessageBoxResults.No)
                                flg = false;
                            }
                        }

                        if (double.TryParse(f.ValueString3, out avg))
                        {
                            avg = double.Parse(f.ValueString3);
                        }
                        else
                        {
                            var ttt = WlstMessageBox.Show("参考电流保存失败", "终端 " + f.Rtuid + " 回路" + f.ValueInt1 + " 参考电流设置有误！是否继续？", WlstMessageBoxType.YesNo);
                            if (ttt == WlstMessageBoxResults.No)
                            {
                                return;
                            }
                            else avg = 0.0;
                        }
                        if (double.TryParse(f.ValueString4, out avg2))
                        {
                            avg2 = double.Parse(f.ValueString4);
                        }
                        else
                        {
                            //var ttt = WlstMessageBox.Show("参考电流二保存失败", "终端 " + f.Rtuid + " 回路" + f.ValueInt1 + " 参考电流二设置有误！是否继续？", WlstMessageBoxType.YesNo);
                            //if (ttt == WlstMessageBoxResults.No)
                            //{
                            //    return;
                            //}
