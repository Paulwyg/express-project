        ...
        Cr.Core.EventHandlerHelper.EventHandlerHelperExtendNotifyProperyChanged, IIEquipmentFaultWithTmlSettingViewModel
    {
        ...
        void FauleCodeNameSelectViewModel_OnSelectChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    if (CurrentSelectItem.IsGroup == false)
                    {
                    ...
                    else
                    {
                        ...
                        foreach (var t in CurrentSelectItem.ChildTreeItems)
                        {
                            bool _ac = true;

                            if (ApplyFault.ContainsKey(t.NodeId) && (ApplyFault[t.NodeId].Count == 1))
                            {
                                foreach (var m in ApplyFault[t.NodeId])
                                {
                                    if (m == 0)
                                    {
                                        _ac = false;
                            ...
                            if (_ac == true)
                            {
                                if (ApplyFault.ContainsKey(0))
                                {
                                    if (ApplyFault[0].Count == lst.Count)
                                    {
                                        ...
                                        if (same)
                                        {
                                            if (ApplyFault.ContainsKey(t.NodeId))
                                                ApplyFault.Remove(t.NodeId);
                                            if (ListTreeTmlNode.Info.ContainsKey(t.NodeId))
                                                ListTreeTmlNode.Info[t.NodeId].IsSelected = false;
                                            continue;
                                        }
                                    }
                                }
                                ...
                                if (!ApplyFault.ContainsKey(t.NodeId))
                                    ApplyFault.Add(t.NodeId, new List<int>());
                                ApplyFault[t.NodeId].Clear();
                                ApplyFault[t.NodeId].AddRange(lst);
                                if (ListTreeTmlNode.Info.ContainsKey(t.NodeId))
                                    ListTreeTmlNode.Info[t.NodeId].IsSelected = true;
                            }
                        }
                    }
                }
        ...
        public NoAlarmViewModel CurrentSelectNoAlarm
        {
            ...
            set
            {
                if (_currentSelectnoalarm != value)
                {
                    ...
                    if (CurrentSelectNoAlarm == null || CurrentSelectNoAlarm.PhyId == 0 )
                    {
                        ShowDelete = "删  除";
                    }
